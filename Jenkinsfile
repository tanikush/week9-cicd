pipeline {
	...
	stage('Docker Push') {
		...
		withCredentials([usernamePassword(credentialId: 'dockerhub-creds', usernameVariable: 'DOCKER_USER', passwordVariable: 'DOCKER_PASS')]) {
	...
	stage('Deploy') {
		...
		sh '''
		    ...
		    docker run -d --name week9-app-new -p 8090:80 $IMAGE_NAME:$IMAGE:TAG
		   '''
	   }
	}
    }
